...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden = torch.nn.Linear(self.n_features * self.embed_size, self.hidden_size)
    ...
    def embedding_lookup(self, t):
        ...
        x = self.pretrained_embeddings(t).view(t.shape[0], self.n_features * self.embed_size)
    ...
    def forward(self, t):
        ...
        logits = self.embedding_lookup(t)
        logits = self.embed_to_hidden(logits)
        logits = torch.nn.ReLU()(logits)
        logits = self.dropout(logits)
        logits = self.hidden_to_logits(logits)
