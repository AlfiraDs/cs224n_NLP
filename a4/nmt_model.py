...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        h = self.hidden_size
        v = len(vocab.tgt)
        self.encoder = nn.LSTM(e, h, bidirectional=True, bias=True)
        self.decoder = nn.LSTMCell(e + h, h, bias=True)
        ...
        self.target_vocab_projection = nn.Linear(h, v, bias=False)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        enc_hiddens, _ = pad_packed_sequence(sequence=enc_hiddens)  # (src_len, b, 2h)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in Y:
            ...
            dec_state, combined_output, e_t = self.step(Ybar_t=Ybar_t,
                                                        dec_state=dec_state,
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = torch.squeeze(torch.bmm(enc_hiddens_proj, torch.unsqueeze(dec_hidden, -1)), dim=-1)  # (b, src_len)
